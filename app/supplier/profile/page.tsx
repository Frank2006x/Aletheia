...
import {
    ...
    Loader2, Truck, BarChart3, ArrowRight,
} from "lucide-react";
        ...
        <div className="min-h-screen bg-[#050505]">
            ...
            <div className="pointer-events-none fixed inset-0 overflow-hidden">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[700px] h-[350px] opacity-[0.05]"
                    style={{ background: `radial-gradient(ellipse, ${INDIGO} 0%, transparent 70%)`, filter: "blur(80px)" }} />
            </div>
            ...
            <div className="sticky top-0 z-10 bg-black/60 backdrop-blur-xl border-b border-white/[0.06] px-4 sm:px-8 py-3 sm:py-4 flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-white/40">Supplier Profile</span>
                </div>
            ...
            >
                ...
                <div className="col-span-12 lg:col-span-4 flex flex-col gap-4">
                    <Card className="bg-[#0c0c0c] border-white/[0.07] rounded-2xl overflow-hidden shadow-xl p-0 gap-0">
                        <div className="h-1" style={{ background: `linear-gradient(to right, ${INDIGO}, rgba(99,102,241,0.3), transparent)` }} />
                        <CardContent className="pt-8 pb-7 flex flex-col items-center text-center px-7">
                            <div className="relative mb-5">
                                <div className="absolute inset-[-4px] rounded-2xl"
                                    style={{ boxShadow: "0 0 22px 5px rgba(99,102,241,0.18)" }} />
                                <Avatar className="w-24 h-24 rounded-2xl ring-2 ring-white/[0.08] relative">
                                    ...
                                    <AvatarFallback className="rounded-2xl text-3xl font-bold"
                                        style={{ background: "rgba(99,102,241,0.1)", color: "rgba(129,140,248,1)" }}>
                                        {initials}
                                    </AvatarFallback>
                                </Avatar>
                                {user.emailVerified && (
                            ...
                            <Badge className="mt-4 items-center gap-1.5 px-3 py-1 border text-xs font-semibold"
                                style={{ background: "rgba(99,102,241,0.1)", borderColor: "rgba(99,102,241,0.25)", color: "rgba(129,140,248,1)" }}>
                                <Truck className="w-3 h-3" /> Supplier
                            </Badge>
                        </CardContent>
                    </Card>

                    <Button onClick={() => router.push("/supplier/dashboard")}
                        className="w-full flex items-center justify-between font-semibold rounded-xl px-5 h-11 group text-white"
